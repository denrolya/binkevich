(function () {
    ...
    angular
        .module('Controllers', ['angularFileUpload'])
        .controller('MultipleFileUploadCtrl', MultipleFileUploadCtrl)
        .controller('MultipleImageViewCtrl', MultipleImageViewCtrl);

    MultipleFileUploadCtrl.$inject = ['$scope', '$http', 'FileUploader'];

    function MultipleFileUploadCtrl($scope, $http, FileUploader) {
        var vm = this;
        ...
        vm.initFileNames = function () {
            ...
            $http.get('/api/v1/media/' + ids + '/name').then(function (response) {
        ...
        vm.removeMedia = function (imageId) {
            $http
                .delete('/product/' + vm.id + '/media/' + imageId)
                .then(function (response) {
                    console.log(vm.medias, vm.medias.indexOf(imageId));
    ...
    MultipleImageViewCtrl.$inject = [];

    function MultipleImageViewCtrl() {
        var vm = this;
